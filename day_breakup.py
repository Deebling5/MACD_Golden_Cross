...
def find_max_high(data, start, end):
    return (data['High'][
            -start:-end - 1:-1].max())  # find the maximum between -2 to end, when the step is negative, the stop index should be less than the start index --> using reverse order is more readable
    ...
    if start <= 1:  # start must be >= 2
        ...
        for i in range(-start, -end - 1, -1):
            ...
            if 1.005 * max_high >= data['High'].iloc[i] >= 0.995 * max_high:  # range of peak: [1.005 to 0.995]
                ...
                # i +=1
                if peak_count >= 4 and data['Close'].iloc[
                    -1] > 1.005 * max_high:  # break up from peak after 4 attempts and consider the retest entry point
                    # print(f"Break up!")
                    return True
...
def downward_wedge(data, start, end, min_low):  # positive start, end (downthrust)
    if start <= 1:  # start must be >= 2
        ...
        for i in range(-start, -end - 1, -1):
            ...
            if 0.995 * min_low <= data['Low'].iloc[
                i] <= 1.005 * min_low:  # range of trough: [1.005 to 0.995]--> it has considered both increasing lower lines and decreasing lower lines
                low_count += 1
                # print(f"Trough @{i} = {data['Low'].iloc[i]}, {low_count}")
                # i +=1

                if low_count >= 4 and data['Close'].iloc[
                    -1] < 0.995 * min_low:  # break down from trough after 4 attempts and consider the retest entry point
                    # print(f"Break down!")
                    return True
...
for stock in os.listdir('data'):
    ...
    try:
        df = pd.read_csv('data/{}'.format(stock))
